...
### Opcodes 0x05, 0x07, 0x08, 0x0B (confirmed from challenge/verify_corrupted.bin)

These four opcodes do not appear in any probe instruction stream. They were identified using `main_corrupted.bin` and `verify_corrupted.bin`.

**Method:** `verify_corrupted.bin` has a instruction `07 04 00 00` using opcode `0x07` to copy a register value, as before no data was loaded in R4 but later ADD R4,R2 is used. Thus confirming `0x07` is used to move data between registers. So **`0x07 = MOV Ra, Rb`** produced correct output.

Also I saw, `main_corrupted` uses:
...
There was no clue for the other three instructions as they were never used as instruction so asked gemini and it gave the following
